...
def matrix_divided(matrix, div):
    ...
    if not isinstance(matrix, list) or #!/usr/bin/python3
"""
This module contains a function that divides elements of matrix by a number
The function checks if the matrix is a list of lists of integers or flaots
checks if the lists of matrix of the same length
...
def matrix_divided(matrix, div):
    ...
    for row in matrix:
        ...
        for row in matrix:
            new_row = []
            for element in row:
                if element == float('inf') or
                element == float('-inf') or element != element:
                    new_row.append(0.0)
                else:
                    new_row.append(round(element / div, 2))
            new_matrix.append(new_row)
    return new_matrixnot all(isinstance(row, list) for row in matrix):
        raise TypeError("matrix must be a matrix (list of lists) of integers/floats")
    if len(matrix) == 1:
        raise TypeError("matrix must be a matrix (list of lists) of integers/floats")
    if not matrix:
    ...
    for row in matrix:
        ...
        for row in matrix:
            new_row = []
            for element in row:
                if element == float('inf') or element == float('-inf') or element != element:
                    new_row.append(0.0)
                else:
                    new_row.append(round(element / div, 2))
            new_matrix.append(new_row)
